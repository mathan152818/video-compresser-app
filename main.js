// ===== Video Compressor — Frontend (Native FFmpeg Backend) =====
...
function handleFile(file) {
  ...
  dom.btnSample.style.display = 'none';
...
async function loadSampleVideo() {
  dom.btnSample.textContent = 'Loading…';
  ...
  try {
    ...
    alert('Failed to load sample video. Please try uploading your own file.');
  } finally {
    dom.btnSample.innerHTML = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg> Try Sample Video (4.4 MB)`;
...
function init() {
  ...
  dom.btnSample.addEventListener('click', loadSampleVideo);
...
async function checkBackend() {
  try {
    ...
    console.warn('Backend not reachable. Make sure server.js is running on port 3001.');
  }
}
